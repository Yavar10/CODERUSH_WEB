...
const rules: Rule[] = [
  ...
  },
  {
    ...
    description:
      "Participate in a team of 3 members only. Team members can be from different colleges.",
    icon: FaUsers,
    color: "#F4C300",
  },
  {
  ...
  return (
    ...
    >
      <div
        className={
          isArcadeMode
            ? "relative overflow-hidden border-3 border-black bg-white shadow-[4px_4px_0px_#000] hover:translate-x-[-2px] hover:translate-y-[-2px] hover:shadow-[6px_6px_0px_#000] transition-all duration-200 h-full p-6 md:p-8"
            : "relative overflow-hidden border border-black/10 bg-white hover:shadow-xl transition-shadow duration-500 h-full p-6 md:p-8"
        }
      >
        <div
          className={
            isArcadeMode
              ? "absolute left-0 top-0 h-full w-2 border-r border-black"
        ...
        <div className="flex items-start gap-4 md:gap-5 pl-2">
          <div
            className={
              isArcadeMode
                ? "shrink-0 flex items-center justify-center h-12 w-12 border-2 border-black font-press-start text-sm shadow-[2px_2px_0px_#000] bg-zinc-50"
                : "shrink-0 flex items-center justify-center h-12 w-12 rounded-xl font-anton text-lg text-white"
            }
            style={{
              backgroundColor: isArcadeMode ? "#fafafa" : rule.color,
              color: isArcadeMode ? rule.color : "#ffffff",
            }}
          >
            {rule.id}
          </div>
          ...
          <div className="flex-1 min-w-0">
            <div className="flex items-center gap-3 mb-3">
              <div
                className={
                  isArcadeMode
                    ? "flex items-center justify-center w-10 h-10 border-2 border-black shrink-0 shadow-[2px_2px_0px_#000]"
              ...
              <h3
                className={
                  isArcadeMode
                    ? "font-press-start text-xs md:text-sm lg:text-base uppercase text-zinc-900 leading-snug tracking-wide"
                    : "font-bold text-lg md:text-xl text-zinc-900"
                }
              >
                {rule.title}
              </h3>
            ...
            {isArcadeMode ? (
              <div className="mt-4 pt-3 border-t-2 border-dashed border-zinc-200 flex items-center justify-between font-silkscreen text-[10px] font-bold text-zinc-400">
                ...
                <span className="text-green-600 font-press-start text-[9px]">
                  [VERIFIED]
                </span>
              </div>
            ) : null}
        ...
        <motion.div
          className={
            isArcadeMode
              ? "absolute bottom-0 left-0 h-[3px] w-full border-t border-black"
  ...
  return (
    ...
    >
      ...
      <div
        className={
          isArcadeMode
            ? "eligibility-tracks relative border-b-2 border-black"
            : "eligibility-tracks relative"
        }
      >
        <div className="mx-auto max-w-7xl px-6 py-4 flex gap-2">
          {olympicColors.map((color, i) => (
            <div
              ...
              className={
                isArcadeMode
                  ? "eligibility-track h-3 flex-1 border border-black shadow-[1px_1px_0px_#000]"
                  : "eligibility-track h-1 flex-1 rounded-full"
              }
      ...
      <div className="relative z-10 mx-auto max-w-7xl px-6 py-20 md:py-32">
        <div className="mb-16 md:mb-24 grid grid-cols-1 lg:grid-cols-12 gap-8 lg:gap-12 items-end">
          <div className="lg:col-span-7">
            <motion.p
              ...
              className={
                isArcadeMode
                  ? "mb-4 font-press-start text-[10px] sm:text-xs font-bold uppercase tracking-widest text-[#8C6A1A]"
                  : "mb-4 text-xs sm:text-sm uppercase tracking-[0.35em] text-[#8C6A1A] font-semibold"
              }
            >
              {isArcadeMode ? "▶ WHO CAN COMPETE" : "Who Can Compete"}
            </motion.p>
            ...
            <motion.h2
              ...
              className={
                isArcadeMode
                  ? "font-press-start text-2xl sm:text-4xl md:text-5xl xl:text-6xl 2xl:text-7xl uppercase text-zinc-900 tracking-tight drop-shadow-[4px_4px_0px_#F4C300]"
                  : "font-anton text-5xl sm:text-6xl md:text-8xl lg:text-[8rem] uppercase leading-[0.9] text-zinc-900"
              }
            >
              ELIGIBILITY
            </motion.h2>
            ...
            <div className="mt-6 flex gap-2">
              {olympicColors.map((color, i) => (
                <motion.div
                  ...
                  className={
                    isArcadeMode
                      ? "h-4 w-4 border border-black shadow-[1px_1px_0px_#000]"
        ...
        <div
          className={
            isArcadeMode
              ? "eligibility-cta relative overflow-hidden border-4 border-black bg-white shadow-[8px_8px_0px_#000] p-8 md:p-14"
              : "eligibility-cta relative overflow-hidden rounded-3xl border border-black/10 bg-white p-8 md:p-14 shadow-xl"
          }
        >
          {isArcadeMode ? (
          ...
          <div className="relative z-10 pt-4 flex flex-col md:flex-row items-center justify-between gap-8">
            <div className="max-w-xl">
              <h3
                className={
                  isArcadeMode
                    ? "font-press-start text-2xl md:text-4xl uppercase text-zinc-900 mb-4 tracking-wide leading-snug"
                    : "font-anton text-3xl md:text-5xl uppercase text-zinc-900 mb-4"
                }
              >
                READY TO
                <br />
                <span
                  className={
                    isArcadeMode
                      ? "text-[#D4AF37] drop-shadow-[2px_2px_0px_#000]"
                      : "text-[#D4AF37]"
                  }
                >
                  COMPETE?
      ...
      <div
        className={
          isArcadeMode ? "relative border-t-2 border-black" : "relative"
        }
      >
        <div className="mx-auto max-w-7xl px-6 py-4 flex gap-2">
          {[...olympicColors].reverse().map((color, i) => (
            <motion.div
              ...
              className={
                isArcadeMode
                  ? "h-2 flex-1 border border-black shadow-[1px_1px_0px_#000]"
